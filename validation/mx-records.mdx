...
for (const email of validDomains) {
  ...
  console.log(`${email}: MX valid = ${result.signals.mx.valid}`);
  console.log(`  Servers: ${result.signals.mx.records.join(', ')}`);
}
```
...
for (const email of invalidDomains) {
  const result = await vouch.validate(email);
  console.log(`${email}: MX valid = ${result.signals.mx.valid}`);
  console.log(`  Reason: ${result.signals.mx.message}`);
}
```
</CodeGroup>
...
if (!result.signals.syntax.valid) {
