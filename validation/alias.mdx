...
if (result.data && !result.data.checks.alias?.pass) {
  // Alias detected
  const fullEmail = result.data.checks.alias?.metadata?.fullEmail; // user+shopping@gmail.com
  const baseEmail = result.data.checks.alias?.metadata?.baseEmail; // user@gmail.com
...
```jsx
{result.data && !result.data.checks.alias?.pass && (
  <Alert variant="info">
    <p>Email aliases (+ addressing) detected.</p>
    <p className="text-sm">
      Please use your base email address: {result.data.checks.alias?.metadata?.baseEmail}
    </p>
  </Alert>
)}
```
...
{result.data && !result.data.checks.alias?.pass && (
  <div>
    ...
    <Button onClick={() => setEmail(result.data.checks.alias?.metadata?.baseEmail)}>
      Use {result.data.checks.alias?.metadata?.baseEmail} instead
    </Button>
  </div>
)}
```
...
for (const email of aliases) {
  ...
  if (result.data && !result.data.checks.alias?.pass) {
    console.log(`${email} → ${result.data.checks.alias?.metadata?.baseEmail}`);
  }
}
```
...
for (const email of regular) {
  const result = await vouch.validate(email);
  if (result.data) {
    console.log(`${email}: alias = ${!result.data.checks.alias?.pass}`);
  }
}
```
</CodeGroup>
