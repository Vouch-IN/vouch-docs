...
```jsx
{result.signals.ipReputation.threats.includes('vpn') && (
...
```jsx
{result.signals.ipReputation.threats.includes('tor') && (
  <Alert variant="error">
...
const ip = result.signals.ipReputation;

console.log(`IP: ${ip.ipAddress}`);
console.log(`Country: ${ip.country}`);
console.log(`ISP: ${ip.isp}`);
console.log(`Threats: ${ip.threats.join(', ')}`);
console.log(`Risk: ${ip.risk}`);

if (ip.threats.includes('vpn')) {
...
// Override IP for server-side validation
const result = await vouch.validate('user@example.com', {
  ip: request.headers['x-forwarded-for'] || request.ip
});

console.log(`Validating from IP: ${result.metadata.ipAddress}`);
```
</CodeGroup>
