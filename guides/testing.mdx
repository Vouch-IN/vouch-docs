...
export const validationFixtures = {
  validEmail: {
    ...
    validations: {
      ...
      disposable: { valid: true, message: 'Not disposable' },
...
describe('Vouch Integration', () => {
  ...
  describe('Valid Emails', () => {
    it('validates Gmail address', async () => {
      ...
      expect(result.valid).toBe(true);
      expect(result.validations.syntax.valid).toBe(true);
      expect(result.validations.mx.valid).toBe(true);
    });
  ...
  describe('Invalid Emails', () => {
    it('blocks disposable email', async () => {
      ...
      expect(result.recommendation).toBe('block');
      expect(result.validations.disposable.valid).toBe(false);
    });
    ...
    it('blocks malformed email', async () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.validations.syntax.valid).toBe(false);
    });
  });
  ...
  describe('Edge Cases', () => {
    it('handles plus addressing', async () => {
      ...
      expect(result.validations.alias).toBeDefined();
    });

    it('detects role emails', async () => {
      const result = await vouch.validate('admin@example.com');

      expect(result.validations.role.valid).toBe(false);
    });
  });
});
```
